...
# Fungsi untuk menampilkan daftar siswa
def tampilkan_siswa(siswa_ditemukan):
    #if not siswa:
    #    print("Daftar siswa kosong.")
    #    return
...
while True:
    ...
    if pilihan == "1":
        ...
        while status != "success":
            print("1. Tampilkan Data Seluruh Siswa")
            print("2. Cari Siswa")
            pilihan = input("Masukkan pilihan Anda: ")
            if pilihan == "1":
                status = tampilkan_siswa()
                continue_ = input("Tekan tombol apapun untuk melanjutkan!...")
                clear_screen()
            elif pilihan == "2":
                nama = input("Masukkan nama siswa yang akan dicari: ")
                siswa_ditemukan, status = cari_siswa(nama)
                if siswa_ditemukan:
                    print("Siswa ditemukan:")
                    status = tampilkan_siswa(siswa_ditemukan)
                else:
        ...
        while status != "success":
            ...
            if siswa_ditemukan:
                ...
                status = perbarui_siswa(nama, data_siswa_baru)
            else:
                print("Siswa tidak ditemukan.")
            continue_ = input("Tekan tombol apapun untuk melanjutkan!...")
            clear_screen()
